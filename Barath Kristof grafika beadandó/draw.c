...
void draw_environment(World world, Rotate* rotate) {
	...
	glNewList(displayList1, GL_COMPILE);
		...
		glBindTexture(GL_TEXTURE_2D, world.grandfatherclock.texture);
		glScalef(20.0f, 20.0f, 20.0f);
    	draw_model(&world.grandfatherclock.model);
	
		
		glPushMatrix();
			glTranslatef(0, 6.9, 0);
			glRotatef(rotate->sec_clock_position, 0, 0, 0);
			glTranslatef(0, -6.9, 0);
	...
	glBindTexture(GL_TEXTURE_2D, world.inga.texture);
...
void load_skybox(Skybox* skybox) {
	...
	skybox->top = load_texture("textures//teto.jpg");
}
