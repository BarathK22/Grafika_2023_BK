...
void calc_rotation_of_objects(Rotate* rotate) {
	...
	if (((int)rotate->valueOfRotationForPendulum < -10 || (int)rotate->valueOfRotationForPendulum > 10) && wholeSec == 1){
		rotate->clock_speed *= -1;
	}
    
	rotate->valueOfRotationForPendulum += rotate->clock_speed;
...
void init() {
	...
	help = load_texture("textures//helpp.png", &world.grandfatherclock.model);
...
int main(int argc, char** argv) {
	...
	int wnd = glutCreateWindow("Grafika");
	glutSetWindow(wnd);
